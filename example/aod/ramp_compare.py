...
Run from the repository root:
    ...
    python3 example/aod/ramp_compare.py [--gif]

Saves the comparison figure (and, with `--gif`, the profile-comparison
animation) into the render/ subdir next to this script.
"""
...
# The run is dark (purely conservative), so only `mass_kg` is read from the
...
# 300 us sits near the lensing-on survival cliff, so the profile choice
# shows up in both survival and heating.
T_DRAG_S = 300.0e-6
T_HOLD_S = 11.0e-6
...
def build_simulation(
...
) -> tuple[AstigmaticAODTrap, RampSequence, SimulationConfig, AtomSystem]:
    ...
    config = SimulationConfig(
        ...
        # The trap moves faster than its escape velocity mid-drag, so the
        # in-flight energy test would flag transiently unbound atoms;
        # survival is recovered post-hoc with `bound_to_trap` at rest.
        energy_loss="off",
        store_trajectories=store_trajectories,
        trajectory_stride=trajectory_stride,
    )
    system = AtomSystem(species=YB171, modules=[aod])
    return aod, ramp, config, system
...
def run_case(case: ProfileCase) -> CaseResult:
    aod, _, config, system = build_simulation(case)
    ...
    if n_final > 0:
        t_init_bound = _kinetic_temperature_uK(
            result.initial_velocities_m_per_s[bound_final_mask], YB171.mass_kg
        )
        t_final_bound = _kinetic_temperature_uK(
            result.final_velocities_m_per_s[bound_final_mask], YB171.mass_kg
        )
    ...
    t_init_all = _kinetic_temperature_uK(
        result.initial_velocities_m_per_s, YB171.mass_kg
    )
    t_final_all = _kinetic_temperature_uK(
        result.final_velocities_m_per_s, YB171.mass_kg
    )
...
def render_comparison_gif(
    ...
    for case in selected:
        aod, ramp, config, system = build_simulation(
            case,
            ensemble_size=ensemble_size,
...
def main() -> None:
    ...
    out_path = os.path.join(RENDER_DIR, "ramp_compare.png")
    ...
    if "--gif" in sys.argv:
        ...
        gif_path = os.path.join(RENDER_DIR, "ramp_compare.gif")
