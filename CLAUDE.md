...
- Tests: `python3 -m unittest discover tests` (or per-module: `tests.test_core`, `tests.test_light_matter` (~1 min — real cooling simulations), `tests.test_ensemble`, `tests.test_physics_modules`)
- Single test: `python3 -m unittest tests.test_core.CorePhysicsTests.test_static_trap_has_low_numerical_heating`
- SLM-to-AOD example: `python3 example/aod/slm_to_aod_transfer.py`; GIF variant `example/aod/transfer_animation.py`
- Spectator-qubit suite: `example/spectator/spectator_qubits.py` (AOD fly-by), `spectator_qubits_ripa.py` (gridded RIPA fly-by), `ripa/generate_ripa_gridded_trap.py` (RIPA trap generator). See [example/spectator/README.md](example/spectator/README.md).
- Light-force examples: `python3 example/mot/rb85_mot.py` (Rb85 MOT; `--backend rate-equation`, `--plot` / `--save-plot`, `--gif`), `python3 example/mot/mmwave_mot.py` (tri-sector grating MOT from a 600 K effusive beam; sector-prism beam profiles via `LaserBeam.profile`), `python3 example/tweezer_probe_heating.py` (mixed trap + scattering)
- New-module example: `python3 example/magnetic_dipole/quadrupole_and_dipole.py` (quadrupole magnetic trap via `ZeemanPotential`; 850 nm `DipoleBeamPotential` tweezer cross-checked against a `GaussianTrap`)
