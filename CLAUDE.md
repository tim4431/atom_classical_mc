...
- SLM-to-AOD example: `python3 example/aod/slm_to_aod_transfer.py` (always writes `_traj` / `_energy` / `_3d` figures into `example/aod/render/`); `example/aod/transfer_animation.py` renders the same transfer as a GIF
- Other examples: `example/spectator/spectator_qubits.py` (AOD fly-by), `example/spectator/spectator_qubits_ripa.py` (gridded RIPA fly-by), `example/spectator/ripa/generate_ripa_gridded_trap.py` (RIPA trap generator), `example/gridded_vs_analytical.py`, `example/astigmatism_check.py` (sanity-checks `AstigmaticAODTrap`). See [example/spectator/README.md](example/spectator/README.md) for the spectator-qubit suite, and [example/README.md](example/README.md) for the layout and the shared `render/`-output convention.
- Light-force examples: `python3 example/mot/rb85_mot.py` (Rb85 MOT capture + cooling; `--backend rate-equation` to switch internal-state backends, `--plot` / `--save-plot` for figures, `--gif` for an animated cloud GIF), `python3 example/mot/mmwave_mot.py` (tri-sector grating MOT loading Rb85 from a 600 K effusive beam; sector-prism beam profiles via `LaserBeam.profile`, `ensemble.EffusiveBeam` flux-distribution initial ensemble), `python3 example/tweezer_probe_heating.py` (recoil heating of a tweezer-trapped atom, mixed conservative + light physics)
...
3. **Initial ensemble** — low-level samplers in [src/sampling.py](src/sampling.py): velocities from Maxwell-Boltzmann; positions from a Gaussian whose covariance is `k_B T · K⁻¹` where `K` is the combined-trap Hessian at the chosen center (deepest trap at `t=0` by default, or `SimulationConfig.initial_center_m`). Named physical *sources* in [src/ensemble.py](src/ensemble.py) sit on top: `EnsembleSource.sample(n, rng) -> EnsembleSample(positions, velocities, weight)`, with `ThermalCloud` (Gaussian/box/sphere cloud — background vapor or pre-cooled cloud), `HarmonicTrapCloud` (the trap-Hessian default as a source), and `EffusiveBeam` (oven with a small hole: flux law `v³exp(−v²/2σ²)` over a truncated speed window, cosine angular law). Pass one via `SimulationConfig.initial_source`; its `weight` (e.g. a beam window's flux fraction) surfaces as `SimulationResult.initial_ensemble_weight`. With `reject_initially_lost=True` (default) the simulator resamples atoms that are unbound at `t=0` so reported loss reflects ramp dynamics, not bad loading. The rejection budget is bounded by `max_initial_resampling_rounds`; exceeding it raises `RuntimeError`.
